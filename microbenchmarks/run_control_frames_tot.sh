...
########## PARAMETERS ##########
echo "Reproduce Scenario: Energy cost of control frames (total energy consumption)"
                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            do
                                echo "threshRTSCTS=$threshRTSCTS;dataSize=$dataSize;topo=$topo;nNode=$nNode;nNodePair=$nNodePair;bitRate=$bitRate;useNS3=$useNS3;radius=$radius;seed=$seed;name=$name;useNS3=${useNS3};simTime=${simTime};corrF=${corrF};upDown=${upDown};biDirection=${biDirection};cancelIDLEEnergy=${cancelIDLEEnergy};pcap=${pcap};mcs=${mcs};" >> $argsFile
                            done
                        done
                    done
                done
...
do
    ...
    currDir=${PWD}
    ssh ${g5k_user}@$host tmux -l new-session -s "simu" -d "${currDir}/run.sh"
    rm $tmp
done
